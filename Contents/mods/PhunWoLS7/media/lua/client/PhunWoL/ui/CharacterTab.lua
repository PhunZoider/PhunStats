...
function UI:drawDatas(y, item, alt)
    ...
    local value = item.item.o and getTextOrNull("UI_PhunWoL_Online") or PhunTools:getTimeDiffAsString(item.item.m)
        ...
        if self:isMouseOver() then
            ...
            if row ~= nil and row > 0 and self.items[row] then
                ...
                if item then
                    ...
                    if item.o then
                        table.insert(desc, getText("UI_PhunWoL_OnlineForX", PhunTools:timeDifferenceAsText(
                            getTimestamp(), item.s, getText("UI_PhunWoL_LessThanAMinute"))))
                    else
                        table.insert(desc, getText("UI_PhunWoL_LastSeenX", PhunTools:timeAgo(item.m)))
                    end
