...
local formatting = require("PhunStats/formating")
...
function UI:drawDatas(y, item, alt)
    ...
    value = formatting:formatWholeNumber(PS.data[self.parent.playerName][self.parent.source][item.item.key] or 0)
...
function TUI:drawDatas(y, item, alt)
    ...
    value = formatting:formatWholeNumber(value)
    local valueWidth = getTextManager():MeasureStringX(self.font, value)
    local w = self.width
    local cw = self.columns[2].size
    self:drawText(value, w - valueWidth - xoffset - 4, y + 4, 1, 1, 1, a, self.font);
